...
void threadWrapper()
{
  ...
  auto e = __expPointer;
  e->run();
  // After the run is finished makes the main execution thread again active
...
void Experiment::initialize()
{
  ...
  __expPointer = this;
  // Allocates a new cothread with 1MB from the Heap Memory for the corthreads stack and
  // makes the function threadWrapper the function to be executed.
  _thread = co_create(1 << 20, threadWrapper);
  // returns and stores a handle to the currently actively running thread
  __returnThread = co_active();
  // Makes _thread the currently active thread and puts __returnThread to sleep
  co_switch(_thread);
...
void Experiment::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Current Generation"))
  {
    try
    {
      _currentGeneration = js["Current Generation"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Is Finished"))
  {
    try
    {
      _isFinished = js["Is Finished"].get<int>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ experiment ] \n + Key:    ['Is Finished']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Timestamp"))
  {
    try
    {
      _timestamp = js["Timestamp"].get<std::string>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ experiment ] \n + Key:    ['Timestamp']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Random Seed"))
  {
    try
    {
      _randomSeed = js["Random Seed"].get<size_t>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ experiment ] \n + Key:    ['Random Seed']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Preserve Random Number Generator States"))
  {
    try
    {
      _preserveRandomNumberGeneratorStates = js["Preserve Random Number Generator States"].get<int>();
    } catch (const std::exception& e) {
 ...
 _problem->setConfiguration(js["Problem"]);
  ...
  if (isDefined(js, "Save Only"))
  {
    try
    {
      _saveOnly = js["Save Only"].get<std::vector<std::string>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ experiment ] \n + Key:    ['Save Only']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Save", "Problem"))
  {
    try
    {
      _saveProblem = js["Save"]["Problem"].get<int>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ experiment ] \n + Key:    ['Save']['Problem']\n%s", e.what());
    }
    eraseValue(js, "Save", "Problem");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Save']['Problem'] required by experiment.\n"); 
 ...
 _solver->setConfiguration(js["Solver"]);
  ...
  if (isDefined(js, "Save", "Solver"))
  {
    try
    {
      _saveSolver = js["Save"]["Solver"].get<int>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ experiment ] \n + Key:    ['Save']['Solver']\n%s", e.what());
    }
    eraseValue(js, "Save", "Solver");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Save']['Solver'] required by experiment.\n"); 

  if (isDefined(js, "File Output", "Path"))
  {
    try
    {
      _fileOutputPath = js["File Output"]["Path"].get<std::string>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "File Output", "Use Multiple Files"))
  {
    try
    {
      _fileOutputUseMultipleFiles = js["File Output"]["Use Multiple Files"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "File Output", "Enabled"))
  {
    try
    {
      _fileOutputEnabled = js["File Output"]["Enabled"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "File Output", "Frequency"))
  {
    try
    {
      _fileOutputFrequency = js["File Output"]["Frequency"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Store Sample Information"))
  {
    try
    {
      _storeSampleInformation = js["Store Sample Information"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Console Output", "Verbosity"))
  {
    try
    {
      _consoleOutputVerbosity = js["Console Output"]["Verbosity"].get<std::string>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Console Output", "Frequency"))
  {
    try
    {
      _consoleOutputFrequency = js["Console Output"]["Frequency"].get<size_t>();
    } catch (const std::exception& e) {
