...
void Recurrent::createHyperparameterMemory()
{
  ...
  _weightsInputCount = L * D * G * OC * IC;
  _weightsRecurrentCount = L * D * G * OC * OC;
  _biasCount = L * D * G * OC;
  _hyperparameterCount = _weightsInputCount + _weightsRecurrentCount + _biasCount;
  ...
  if (_nn->_engine == "OneDNN")
  {
    ...
    _weightsInputDims = {L, D, IC, G, OC};
    auto weightInputMemDesc = memory::desc(_weightsInputDims, memory::data_type::f32, memory::format_tag::ldigo);
    _weightsLayerMem = memory(weightInputMemDesc, _nn->_dnnlEngine);

    _weightsRecurrentDims = {L, D, OC, G, OC};
    auto weightRecurrentMemDesc = memory::desc(_weightsRecurrentDims, memory::data_type::f32, memory::format_tag::ldigo);
    _weightsRecurrentMem = memory(weightRecurrentMemDesc, _nn->_dnnlEngine);

    _biasDims = {L, D, G, OC};
    auto biasMemDesc = memory::desc(_biasDims, memory::data_type::f32, memory::format_tag::ldgo);
...
void Recurrent::copyHyperparameterPointers(Layer *dstLayer)
{
  ...
  // Parameter Counts
  dstPtr->_weightsInputCount = _weightsInputCount;
  dstPtr->_weightsRecurrentCount = _weightsRecurrentCount;
  ...
  if (_nn->_engine == "OneDNN")
  {
    dstPtr->_weightsInputDims = _weightsInputDims;
    dstPtr->_weightsRecurrentDims = _weightsRecurrentDims;
    dstPtr->_biasDims = _biasDims;
    // Memory
