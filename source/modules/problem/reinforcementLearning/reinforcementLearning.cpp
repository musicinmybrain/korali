...
size_t ReinforcementLearning::getTeamIndex(const size_t agentId)
{
  size_t bufferTeam;
  size_t bufferSum;

  bufferSum = _agentsPerTeam[0];

  if(agentId < bufferSum) bufferTeam = 0;

  for (size_t i = 0; i < _agentsPerTeam.size() - 1; i++)
  {
    if((agentId < (bufferSum + _agentsPerTeam[i+1])) && (agentId > bufferSum - 1)) 
      bufferTeam = i+1;
    bufferSum += _agentsPerTeam[i+1];
  }
...
void ReinforcementLearning::runEnvironment(Sample &worker)
{
  ...
  for (size_t i = 0; i < _agentsPerEnvironment; i++)
  {
    ...
    if (worker["State"][i].size() != _stateVectorSize[teamId]) KORALI_LOG_ERROR("Agents state vector %lu returned with the wrong size: %lu, expected: %lu.\n", i, worker["State"][i].size(), _stateVectorSize[teamId]);

    for (size_t j = 0; j < _stateVectorSize[teamId]; j++)
  ...
  for (size_t i = 0; i < _agentsPerEnvironment; i++)
  {
    ...
    // Scale the state
    for (size_t d = 0; d < _stateVectorSize[teamId]; ++d)
  ...
  // TODO: Suboptimal if available actions are not used
  ...
  if (not isDefined(worker._js.getJson(), "Available Actions"))
  {
    ...
    {
      size_t teamId = getTeamIndex(i);
      worker["Available Actions"][i] = availableActions[teamId];
    }
  }
  ...
  {
    size_t teamId = getTeamIndex(i);
    if (worker["Available Actions"][i].size() != _actionCount[teamId]) KORALI_LOG_ERROR("Available Actions vector %lu returned with the wrong size: %lu, expected: %lu.\n", i, worker["Available Actions"][i].size(), _actionCount[teamId]);
  }
}
