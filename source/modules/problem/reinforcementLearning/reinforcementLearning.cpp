...
void ReinforcementLearning::runTestingEpisode(Sample &worker)
{
  ...
  // Calculating average reward over episodes and agents
...
void ReinforcementLearning::runEnvironment(Sample &worker)
{
  ...
  auto stateRescalingMeans = worker["State Rescaling"]["Means"].get<std::vector<std::vector<float>>>();
  auto stateRescalingSdevs = worker["State Rescaling"]["Standard Deviations"].get<std::vector<std::vector<float>>>();

  // Define feature rescaling variables
  auto featureRescalingMeans = worker["Feature Rescaling"]["Means"].get<std::vector<std::vector<float>>>();
  auto featureRescalingSdevs = worker["Feature Rescaling"]["Standard Deviations"].get<std::vector<std::vector<float>>>();
  ...
  for (size_t i = 0; i < _agentsPerEnvironment; i++)
  {
    ...
    auto features = worker["Features"][i].get<std::vector<float>>();

    // Scale the features
    for (size_t d = 0; d < _featureVectorSize; ++d)
      features[d] = (features[d] - featureRescalingMeans[i][d]) / featureRescalingSdevs[i][d];
