...
void ReinforcementLearning::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Action Vector Size"))
  {
    try
    {
      _actionVectorSize = js["Action Vector Size"].get<size_t>();
  ...
  if (isDefined(js, "State Vector Size"))
  {
    try
    {
      _stateVectorSize = js["State Vector Size"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Action Vector Indexes"))
  {
    try
    {
      _actionVectorIndexes = js["Action Vector Indexes"].get<std::vector<size_t>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "State Vector Indexes"))
  {
    try
    {
      _stateVectorIndexes = js["State Vector Indexes"].get<std::vector<size_t>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Action Count"))
  {
    try
    {
      _actionCount = js["Action Count"].get<size_t>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ reinforcementLearning ] \n + Key:    ['Action Count']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Agents Per Environment"))
  {
    try
    {
      _agentsPerEnvironment = js["Agents Per Environment"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Policies Per Environment"))
  {
    try
    {
      _policiesPerEnvironment = js["Policies Per Environment"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Environment Count"))
  {
    try
    {
      _environmentCount = js["Environment Count"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Environment Function"))
  {
    try
    {
      _environmentFunction = js["Environment Function"].get<std::uint64_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Actions Between Policy Updates"))
  {
    try
    {
      _actionsBetweenPolicyUpdates = js["Actions Between Policy Updates"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing Frequency"))
  {
    try
    {
      _testingFrequency = js["Testing Frequency"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Policy Testing Episodes"))
  {
    try
    {
      _policyTestingEpisodes = js["Policy Testing Episodes"].get<size_t>();
    } catch (const std::exception& e) {
 ...
 for (size_t i = 0; i < _k->_js["Variables"].size(); i++) { 
  if (isDefined(_k->_js["Variables"][i], "Type"))
  {
    try
    {
      _k->_variables[i]->_type = _k->_js["Variables"][i]["Type"].get<std::string>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ reinforcementLearning ] \n + Key:    ['Type']\n%s", e.what());
    }
      {
  ...
  if (isDefined(_k->_js["Variables"][i], "Lower Bound"))
  {
    try
    {
      _k->_variables[i]->_lowerBound = _k->_js["Variables"][i]["Lower Bound"].get<float>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ reinforcementLearning ] \n + Key:    ['Lower Bound']\n%s", e.what());
    }
  ...
  if (isDefined(_k->_js["Variables"][i], "Upper Bound"))
  {
    try
    {
      _k->_variables[i]->_upperBound = _k->_js["Variables"][i]["Upper Bound"].get<float>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ reinforcementLearning ] \n + Key:    ['Upper Bound']\n%s", e.what());
    }
