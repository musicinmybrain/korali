...
void ReinforcementLearning::runTrainingEpisode(Sample &worker)
{
  ...
  while (worker["Termination"] == "Non Terminal")
  {
    ...
    episode["Experiences"][actionCount]["State"] = KORALI_GET(std::vector<float>, worker, "State");

    // Storing the current action
    episode["Experiences"][actionCount]["Action"] = KORALI_GET(std::vector<float>, worker, "Action");
  ...
  if (worker.contains("Mini Batch"))
    if (worker.contains("Distribution Params") == false)
      KORALI_LOG_ERROR("State sequence not evaluated in environment. Sample object does not contain Distribution Params");

  if ((worker.contains("Mini Batch") and worker.contains("Distribution Params")) == true)
  {
    ...
    const auto distributionParamsList = KORALI_GET(std::vector<std::vector<std::vector<float>>>, worker, "Distribution Params");

    if (miniBatchList.size() != distributionParamsList.size())
        KORALI_LOG_ERROR("Distribution Params length (%zu) does not match Mini Batch length (%zu)", distributionParamsList.size(), miniBatchList.size());

    if (distributionParamsList[0][0].size() != 2 * _actionVectorSize)
      KORALI_LOG_ERROR("Distribution Params must be of size %zu but are of size %zu", 2 * _actionVectorSize, distributionParamsList[0][0].size());

    message["Mini Batch"] = miniBatchList;
    message["Distribution Params"] = distributionParamsList;
  }
...
void ReinforcementLearning::runTestingEpisode(Sample &worker)
{
  ...
  while (worker["Termination"] == "Non Terminal")
  {
    ...
    if (_agentsPerEnvironment == 1)
    {
      const auto action = KORALI_GET(std::vector<float>, worker, "Action");
      worker._js.getJson().erase("Action");
      worker["Action"][0] = action;
    }
