...
void Propagation::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Execution Model"))
  {
    try
    {
      _executionModel = js["Execution Model"].get<std::uint64_t>();
  ...
  if (isDefined(js, "Number Of Samples"))
  {
    try
    {
      _numberOfSamples = js["Number Of Samples"].get<size_t>();
    } catch (const std::exception& e) {
 ...
 for (size_t i = 0; i < _k->_js["Variables"].size(); i++) { 
  if (isDefined(_k->_js["Variables"][i], "Precomputed Values"))
  {
    try
    {
      _k->_variables[i]->_precomputedValues = _k->_js["Variables"][i]["Precomputed Values"].get<std::vector<double>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(_k->_js["Variables"][i], "Prior Distribution"))
  {
    try
    {
      _k->_variables[i]->_priorDistribution = _k->_js["Variables"][i]["Prior Distribution"].get<std::string>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(_k->_js["Variables"][i], "Sampled Values"))
  {
    try
    {
      _k->_variables[i]->_sampledValues = _k->_js["Variables"][i]["Sampled Values"].get<std::vector<double>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ propagation ] \n + Key:    ['Sampled Values']\n%s", e.what());
    }
