...
void DeepSupervisor::printGenerationAfter()
{
  if (_mode == "Automatic Training")
  {
    ...
    if(_hasValidationSet)
      _k->_logger->logInfo("Normal", "\r[Korali] Epoch %zu/%zu %s Train Loss: %f | Val. Loss: %f | Time %.3fs | Learning Rate: %f\r", _epochCount, _epochs, bar, _currentTrainingLoss, _currentValidationLoss, _k->_genTime.back(), _optimizer->_eta);
    else
      _k->_logger->logInfo("Normal", "\r[Korali] Epoch %zu/%zu %s Train Loss: %f | Time %.3fs | Learning Rate: %f\r", _epochCount, _epochs, bar, _currentTrainingLoss, _k->_genTime.back(), _optimizer->_eta);
...
void DeepSupervisor::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Current Metrics"))
  {
    try
    {
      _currentMetrics = js["Current Metrics"].get<float>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ deepSupervisor ] \n + Key:    ['Current Metrics']\n%s", e.what());
    }
    eraseValue(js, "Current Metrics");
  }
  ...
  if (isDefined(js, "Total Metrics"))
  {
    try
    {
      _totalMetrics = js["Total Metrics"].get<std::vector<float>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ deepSupervisor ] \n + Key:    ['Total Metrics']\n%s", e.what());
    }
    eraseValue(js, "Total Metrics");
  }
...
void DeepSupervisor::getConfiguration(knlohmann::json& js) 
{
  ...
  js["Type"] = _type;
   ...
   js["Current Metrics"] = _currentMetrics;
   js["Training Loss"] = _trainingLoss;
   js["Validation Loss"] = _validationLoss;
   js["Total Metrics"] = _totalMetrics;
