...
void DeepSupervisor::printGenerationAfter()
{
  if (_mode == "Automatic Training")
  {
    ...
    if(_hasValidationSet)
      _k->_logger->logInfo("Normal", "\r[Korali] Epoch %zu/%zu %s Train Loss: %f | Val. Loss: %f | Time %.3fs | Learning Rate: %f\r", _epochCount, _epochs, bar, _currentTrainingLoss, _currentValidationLoss, _k->_genTime, _optimizer->_eta);
    else
      _k->_logger->logInfo("Normal", "\r[Korali] Epoch %zu/%zu %s Train Loss: %f | Time %.3fs | Learning Rate: %f\r", _epochCount, _epochs, bar, _currentTrainingLoss, _k->_genTime, _optimizer->_eta);
