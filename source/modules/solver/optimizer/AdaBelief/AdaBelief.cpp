...
void AdaBelief::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Current Variable"))
  {
    try
    {
      _currentVariable = js["Current Variable"].get<std::vector<double>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Current Variable']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Gradient"))
  {
    try
    {
      _gradient = js["Gradient"].get<std::vector<double>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Gradient']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Best Ever Gradient"))
  {
    try
    {
      _bestEverGradient = js["Best Ever Gradient"].get<std::vector<double>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Gradient Norm"))
  {
    try
    {
      _gradientNorm = js["Gradient Norm"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Gradient Norm']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Bias Corrected First Moment"))
  {
    try
    {
      _biasCorrectedFirstMoment = js["Bias Corrected First Moment"].get<std::vector<double>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Bias Corrected Second Central Moment"))
  {
    try
    {
      _biasCorrectedSecondCentralMoment = js["Bias Corrected Second Central Moment"].get<std::vector<double>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Bias Corrected Second Central Moment']\n%s", e.what());
    }
    eraseValue(js, "Bias Corrected Second Central Moment");
  }
  if (isDefined(js, "Beta1"))
  {
    try
    {
      _beta1 = js["Beta1"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Beta1']\n%s", e.what());
    }
    eraseValue(js, "Beta1");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Beta1'] required by AdaBelief.\n"); 

  if (isDefined(js, "Beta2"))
  {
    try
    {
      _beta2 = js["Beta2"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Beta2']\n%s", e.what());
    }
    eraseValue(js, "Beta2");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Beta2'] required by AdaBelief.\n"); 

  if (isDefined(js, "Eta"))
  {
    try
    {
      _eta = js["Eta"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Eta']\n%s", e.what());
    }
    eraseValue(js, "Eta");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Eta'] required by AdaBelief.\n"); 

  if (isDefined(js, "Epsilon"))
  {
    try
    {
      _epsilon = js["Epsilon"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Epsilon']\n%s", e.what());
    }
    eraseValue(js, "Epsilon");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Epsilon'] required by AdaBelief.\n"); 

  if (isDefined(js, "Termination Criteria", "Min Gradient Norm"))
  {
    try
    {
      _minGradientNorm = js["Termination Criteria"]["Min Gradient Norm"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Termination Criteria']['Min Gradient Norm']\n%s", e.what());
    }
    eraseValue(js, "Termination Criteria", "Min Gradient Norm");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Termination Criteria']['Min Gradient Norm'] required by AdaBelief.\n"); 

  if (isDefined(js, "Termination Criteria", "Max Gradient Norm"))
  {
    try
    {
      _maxGradientNorm = js["Termination Criteria"]["Max Gradient Norm"].get<double>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ AdaBelief ] \n + Key:    ['Termination Criteria']['Max Gradient Norm']\n%s", e.what());
    }
    eraseValue(js, "Termination Criteria", "Max Gradient Norm");
  }  else  KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Termination Criteria']['Max Gradient Norm'] required by AdaBelief.\n"); 

 if (isDefined(_k->_js.getJson(), "Variables"))
 for (size_t i = 0; i < _k->_js["Variables"].size(); i++) { 
 } 
 Optimizer::setConfiguration(js);
 _type = "optimizer/AdaBelief";
 if(isDefined(js, "Type")) eraseValue(js, "Type");
 if(isEmpty(js) == false) KORALI_LOG_ERROR(" + Unrecognized settings for Korali module: AdaBelief: \n%s\n", js.dump(2).c_str());
} 

void AdaBelief::getConfiguration(knlohmann::json& js) 
{
  ...
  js["Type"] = _type;
   ...
   js["Bias Corrected Second Central Moment"] = _biasCorrectedSecondCentralMoment;
 Optimizer::getConfiguration(js);
} 

void AdaBelief::applyModuleDefaults(knlohmann::json& js) 
{
...
void AdaBelief::applyVariableDefaults() 
{

 Optimizer::applyVariableDefaults();
} 

bool AdaBelief::checkTermination()
{
  bool hasFinished = false;

  if ((_k->_currentGeneration > 1) && (_gradientNorm <= _minGradientNorm))
  {
    _terminationCriteria.push_back("AdaBelief['Min Gradient Norm'] = " + std::to_string(_minGradientNorm) + ".");
    hasFinished = true;
  }

  if ((_k->_currentGeneration > 1) && (_gradientNorm >= _maxGradientNorm))
  {
    _terminationCriteria.push_back("AdaBelief['Max Gradient Norm'] = " + std::to_string(_maxGradientNorm) + ".");
    hasFinished = true;
  }
