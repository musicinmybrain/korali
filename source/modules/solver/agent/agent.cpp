...
void Agent::initialize()
{
  ...
  if (_k->_currentGeneration == 0)
  {
    ...
    _stateRescalingMeans.resize(numAgents);
    _stateRescalingSigmas.resize(numAgents);
...
void Agent::rescaleStates()
{
  ...
  std::vector<std::vector<float>> sumStates;
  std::vector<std::vector<float>> squaredSumStates;
...
void Agent::processEpisode(knlohmann::json &episode)
{
  ...
  if (_terminationBuffer[endId] == e_truncated)
  {
    for (size_t a = 0; a < numAgents; a++)
    {
      ...
      const size_t teamId = _problem->getTeamIndex(a);
      ...
      if (_problem->_policiesPerEnvironment == 1)
        ...
        retV[a] = calculateStateValue(expTruncatedStateSequence, teamId);
...
#pragma omp parallel for reduction(vec_int_plus \
  ...
  for (size_t i = 0; i < updateMinibatch.size(); i++)
  {
    ...
    const float importanceWeight = calculateImportanceWeight(expAction, curPolicy, expPolicy, teamId);
    ...
    if (_terminationBuffer[expId] == e_truncated)
    {
      ...
      if (_problem->_policiesPerEnvironment == 1)
        ...
        truncatedStateValue = calculateStateValue(expTruncatedStateSequence, teamId);
