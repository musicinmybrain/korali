...
void Agent::initialize()
{
  ...
  _retraceValueVectorContiguous.resize(_experienceReplayMaximumSize * numAgents);
  _rewardVectorContiguous.resize(_experienceReplayMaximumSize * numAgents);
  _stateValueVectorContiguous.resize(_experienceReplayMaximumSize * numAgents);
  _importanceWeightVector.resize(_experienceReplayMaximumSize);
  _truncatedImportanceWeightVectorContiguous.resize(_experienceReplayMaximumSize * numAgents);
...
void Agent::rescaleStates()
{
  ...
  for (size_t i = 0; i < _stateVector.size(); ++i)
    for (size_t a = 0; a < _problem->_agentsPerEnvironment; ++a)
      ...
      {
        sumStates[a][d] += _stateVector[i][a][d];
        squaredSumStates[a][d] += _stateVector[i][a][d] * _stateVector[i][a][d];
      }
  ...
  for (size_t a = 0; a < _problem->_agentsPerEnvironment; ++a)
    for (size_t d = 0; d < _problem->_stateVectorSize; ++d)
    {
      _stateRescalingMeans[a][d] = sumStates[a][d] / (float)_stateVector.size();
      if (std::isfinite(_stateRescalingMeans[a][d]) == false) _stateRescalingMeans[a][d] = 0.0f;

      _stateRescalingSigmas[a][d] = std::sqrt(squaredSumStates[a][d] / (float)_stateVector.size() - _stateRescalingMeans[a][d] * _stateRescalingMeans[a][d]);
  ...
  for (size_t i = 0; i < _stateVector.size(); ++i)
    for (size_t a = 0; a < _problem->_agentsPerEnvironment; ++a)
      for (size_t d = 0; d < _problem->_stateVectorSize; ++d)
        _stateVector[i][a][d] = (_stateVector[i][a][d] - _stateRescalingMeans[a][d]) / _stateRescalingSigmas[a][d];
}
...
void Agent::processEpisode(knlohmann::json &episode)
{
  ...
  for (size_t expId = 0; expId < episodeExperienceCount; expId++)
  {
    ...
    {
      if (_rewardVectorContiguous.size() >= _experienceReplayMaximumSize * numAgents)
      {
        for (size_t a = 0; a < numAgents; a++)
    ...
    for (size_t a = 0; a < numAgents; a++)
      _rewardVectorContiguous.add(reward[a]);
    ...
    for (size_t a = 0; a < numAgents; a++)
      _stateValueVectorContiguous.add(stateValue[a]);
    ...
    for (size_t a = 0; a < numAgents; a++)
      _retraceValueVectorContiguous.add(0.0f);
    ...
    if (_isOnPolicyVector.size() == _experienceReplayMaximumSize)
    {
      ...
      for (size_t a = 0; a < numAgents; a++)
        if ((onPolicyVector[a] == false) || (_problem->_policiesPerEnvironment == 1))
        {
    ...
    _isOnPolicyVector.add(std::vector<char>(numAgents, true));

    // Initialize experience's importance weight (1.0 because its freshly produced)
    _importanceWeightVector.add(std::vector<float>(numAgents, 1.0f));
    for (size_t a = 0; a < numAgents; a++)
      _truncatedImportanceWeightVectorContiguous.add(1.0f);
  ...
  for (ssize_t expId = endId; expId >= startId; expId--)
    for (size_t a = 0; a < numAgents; a++)
    {
      ...
      retV[a] = getScaledReward(_rewardVectorContiguous[expId * numAgents + a]) + _discountFactor * retV[a];
      _retraceValueVectorContiguous[expId * numAgents + a] = retV[a];
    }

  // Update reward rescaling sigma
  if (_rewardRescalingEnabled)
    _rewardRescalingSigma = std::sqrt(_rewardRescalingSumSquaredRewards / ((float)_rewardVectorContiguous.size()) + 1e-9);
}
...
#pragma omp parallel for reduction(vec_int_plus \
  ...
  for (size_t i = 0; i < updateMinibatch.size(); i++)
  {
    ...
    // Get state value
    _stateValueVectorContiguous[expId * numAgents + agentId] = curPolicy.stateValue;
    ...
    _truncatedImportanceWeightVectorContiguous[expId * numAgents + agentId] = std::min(_importanceWeightTruncationLevel, importanceWeight);
...
#pragma omp parallel for reduction(vec_int_plus \
    ...
    for (size_t i = 0; i < updateBatch.size(); i++)
    {
      ...
      for (size_t a = 0; a < numAgents; a++)
        _truncatedImportanceWeightVectorContiguous[expId * numAgents + a] = truncatedProdImportanceWeight;
...
#pragma omp parallel for
    for (size_t i = 0; i < updateBatch.size(); i++)
    {
      ...
      for (size_t a = 0; a < numAgents; a++)
        averageStateValue += _stateValueVectorContiguous[expId * numAgents + a];
      averageStateValue /= numAgents;

      // Overwrite state value with average
      for (size_t a = 0; a < numAgents; a++)
        _stateValueVectorContiguous[expId * numAgents + a] = averageStateValue;
  ...
  for (size_t a = 0; a < numAgents; a++)
  {
    ...
    _experienceReplayOffPolicyRatio[a] = (float)_experienceReplayOffPolicyCount[a] / (float)_isOnPolicyVector.size();
...
#pragma omp parallel for schedule(guided, 1)
  for (size_t i = 0; i < retraceMiniBatch.size(); i++)
  {
    ...
    if (_terminationVector[endId] == e_nonTerminal)
      for (size_t a = 0; a < numAgents; a++)
        retV[a] = _retraceValueVectorContiguous[(endId + 1) * numAgents + a];
    ...
    for (ssize_t curId = endId; curId >= startId; curId--)
      for (size_t a = 0; a < numAgents; a++)
      {
        ...
        const float truncatedImportanceWeight = _truncatedImportanceWeightVectorContiguous[curId * numAgents + a];

        // Load state value
        const float stateValue = _stateValueVectorContiguous[curId * numAgents + a];

        // Getting current reward, action, and state
        const float curReward = getScaledReward(_rewardVectorContiguous[curId * numAgents + a]);

        // Apply recursion
        retV[a] = stateValue + truncatedImportanceWeight * (curReward + _discountFactor * retV[a] - stateValue);

        // Store retrace value
        _retraceValueVectorContiguous[curId * numAgents + a] = retV[a];
      }
  }
}
...
void Agent::serializeExperienceReplay()
{
  ...
  for (size_t i = 0; i < _stateVector.size(); i++)
  {
    ...
    for (size_t a = 0; a < numAgents; a++)
    {
      stateJson["Experience Replay"][i]["Reward"][a] = _rewardVectorContiguous[i * numAgents + a];
      stateJson["Experience Replay"][i]["State Value"][a] = _stateValueVectorContiguous[i * numAgents + a];
      stateJson["Experience Replay"][i]["Retrace Value"][a] = _retraceValueVectorContiguous[i * numAgents + a];
      stateJson["Experience Replay"][i]["Truncated Importance Weight"][a] = _truncatedImportanceWeightVectorContiguous[i * numAgents + a];
    }

    std::vector<float> expStateValue(numAgents, 0.0f);
    std::vector<std::vector<float>> expDistributionParameter(numAgents, std::vector<float>(_expPolicyVector[0][0].distributionParameters.size()));
    std::vector<size_t> expActionIdx(numAgents, 0);
    std::vector<std::vector<float>> expUnboundedAct(numAgents, std::vector<float>(_expPolicyVector[0][0].unboundedAction.size()));
    std::vector<std::vector<float>> expActProb(numAgents, std::vector<float>(_expPolicyVector[0][0].actionProbabilities.size()));
    std::vector<std::vector<size_t>> expAvailAct(numAgents, std::vector<size_t>(_expPolicyVector[0][0].availableActions.size()));

    std::vector<float> curStateValue(numAgents, 0.0f);
    std::vector<std::vector<float>> curDistributionParameter(numAgents, std::vector<float>(_curPolicyVector[0][0].distributionParameters.size()));
    std::vector<size_t> curActionIdx(numAgents, 0);
    std::vector<std::vector<float>> curUnboundedAct(numAgents, std::vector<float>(_curPolicyVector[0][0].unboundedAction.size()));
    std::vector<std::vector<float>> curActProb(numAgents, std::vector<float>(_curPolicyVector[0][0].actionProbabilities.size()));
    std::vector<std::vector<size_t>> curAvailAct(numAgents, std::vector<size_t>(_curPolicyVector[0][0].availableActions.size()));

    for (size_t a = 0; a < numAgents; a++)
    {
      expStateValue[a] = _expPolicyVector[i][a].stateValue;
      expDistributionParameter[a] = _expPolicyVector[i][a].distributionParameters;
      expActionIdx[a] = _expPolicyVector[i][a].actionIndex;
      expUnboundedAct[a] = _expPolicyVector[i][a].unboundedAction;
      expActProb[a] = _expPolicyVector[i][a].actionProbabilities;
      expAvailAct[a] = _expPolicyVector[i][a].availableActions;

      curStateValue[a] = _curPolicyVector[i][a].stateValue;
      curDistributionParameter[a] = _curPolicyVector[i][a].distributionParameters;
      curActionIdx[a] = _curPolicyVector[i][a].actionIndex;
      curUnboundedAct[a] = _curPolicyVector[i][a].unboundedAction;
      curActProb[a] = _curPolicyVector[i][a].actionProbabilities;
      curAvailAct[a] = _curPolicyVector[i][a].availableActions;
    }
...
void Agent::deserializeExperienceReplay()
{
  ...
  for (size_t i = 0; i < stateJson["Experience Replay"].size(); i++)
  {
    ...
    for (size_t a = 0; a < numAgents; a++)
    {
      _rewardVectorContiguous.add(stateJson["Experience Replay"][i]["Reward"][a].get<float>());
      _stateValueVectorContiguous.add(stateJson["Experience Replay"][i]["State Value"][a].get<float>());
      _retraceValueVectorContiguous.add(stateJson["Experience Replay"][i]["Retrace Value"][a].get<float>());
      _truncatedImportanceWeightVectorContiguous.add(stateJson["Experience Replay"][i]["Truncated Importance Weight"][a].get<float>());
    }
...
void Agent::printGenerationAfter()
{
  if (_mode == "Training")
  {
    ...
    for (size_t a = 0; a < _problem->_agentsPerEnvironment; a++)
    {
      ...
      _k->_logger->logInfo("Normal", " + Count (Ratio/Target):        %lu/%lu (%.3f/%.3f)\n", numPolicies > 1 ? _experienceReplayOffPolicyCount[a] : _experienceReplayOffPolicyCount[a] / _problem->_agentsPerEnvironment, _stateVector.size(), _experienceReplayOffPolicyRatio[a], _experienceReplayOffPolicyTarget);
...
void Agent::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Policy", "Parameter Count"))
  {
    try
    {
      _policyParameterCount = js["Policy"]["Parameter Count"].get<size_t>();
  ...
  if (isDefined(js, "Action Lower Bounds"))
  {
    try
    {
      _actionLowerBounds = js["Action Lower Bounds"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Action Upper Bounds"))
  {
    try
    {
      _actionUpperBounds = js["Action Upper Bounds"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Current Episode"))
  {
    try
    {
      _currentEpisode = js["Current Episode"].get<size_t>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ agent ] \n + Key:    ['Current Episode']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Training", "Reward History"))
  {
    try
    {
      _trainingRewardHistory = js["Training"]["Reward History"].get<std::vector<std::vector<float>>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Training", "Experience History"))
  {
    try
    {
      _trainingExperienceHistory = js["Training"]["Experience History"].get<std::vector<size_t>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Average Reward History"))
  {
    try
    {
      _testingAverageRewardHistory = js["Testing"]["Average Reward History"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Training", "Average Reward"))
  {
    try
    {
      _trainingAverageReward = js["Training"]["Average Reward"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Training", "Last Reward"))
  {
    try
    {
      _trainingLastReward = js["Training"]["Last Reward"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Training", "Best Reward"))
  {
    try
    {
      _trainingBestReward = js["Training"]["Best Reward"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Training", "Best Episode Id"))
  {
    try
    {
      _trainingBestEpisodeId = js["Training"]["Best Episode Id"].get<std::vector<size_t>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Reward"))
  {
    try
    {
      _testingReward = js["Testing"]["Reward"].get<std::vector<float>>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ agent ] \n + Key:    ['Testing']['Reward']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Testing", "Best Reward"))
  {
    try
    {
      _testingBestReward = js["Testing"]["Best Reward"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Worst Reward"))
  {
    try
    {
      _testingWorstReward = js["Testing"]["Worst Reward"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Best Episode Id"))
  {
    try
    {
      _testingBestEpisodeId = js["Testing"]["Best Episode Id"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Candidate Count"))
  {
    try
    {
      _testingCandidateCount = js["Testing"]["Candidate Count"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Average Reward"))
  {
    try
    {
      _testingAverageReward = js["Testing"]["Average Reward"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Best Average Reward"))
  {
    try
    {
      _testingBestAverageReward = js["Testing"]["Best Average Reward"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "Count"))
  {
    try
    {
      _experienceReplayOffPolicyCount = js["Experience Replay"]["Off Policy"]["Count"].get<std::vector<size_t>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "Ratio"))
  {
    try
    {
      _experienceReplayOffPolicyRatio = js["Experience Replay"]["Off Policy"]["Ratio"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "Current Cutoff"))
  {
    try
    {
      _experienceReplayOffPolicyCurrentCutoff = js["Experience Replay"]["Off Policy"]["Current Cutoff"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "REFER Current Beta"))
  {
    try
    {
      _experienceReplayOffPolicyREFERCurrentBeta = js["Experience Replay"]["Off Policy"]["REFER Current Beta"].get<std::vector<float>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Current Learning Rate"))
  {
    try
    {
      _currentLearningRate = js["Current Learning Rate"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Policy Update Count"))
  {
    try
    {
      _policyUpdateCount = js["Policy Update Count"].get<size_t>();
    } catch (const std::exception& e) {
 ...
 _uniformGenerator->setConfiguration(js["Uniform Generator"]);
  ...
  if (isDefined(js, "Experience Count"))
  {
    try
    {
      _experienceCount = js["Experience Count"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Reward", "Rescaling", "Sigma"))
  {
    try
    {
      _rewardRescalingSigma = js["Reward"]["Rescaling"]["Sigma"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Reward", "Rescaling", "Sum Squared Rewards"))
  {
    try
    {
      _rewardRescalingSumSquaredRewards = js["Reward"]["Rescaling"]["Sum Squared Rewards"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "State Rescaling", "Means"))
  {
    try
    {
      _stateRescalingMeans = js["State Rescaling"]["Means"].get<std::vector<std::vector<float>>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "State Rescaling", "Sigmas"))
  {
    try
    {
      _stateRescalingSigmas = js["State Rescaling"]["Sigmas"].get<std::vector<std::vector<float>>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Effective Minibatch Size"))
  {
    try
    {
      _effectiveMinibatchSize = js["Effective Minibatch Size"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Testing", "Sample Ids"))
  {
    try
    {
      _testingSampleIds = js["Testing"]["Sample Ids"].get<std::vector<size_t>>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Training", "Average Depth"))
  {
    try
    {
      _trainingAverageDepth = js["Training"]["Average Depth"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Concurrent Environments"))
  {
    try
    {
      _concurrentEnvironments = js["Concurrent Environments"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Episodes Per Generation"))
  {
    try
    {
      _episodesPerGeneration = js["Episodes Per Generation"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Mini Batch", "Size"))
  {
    try
    {
      _miniBatchSize = js["Mini Batch"]["Size"].get<size_t>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ agent ] \n + Key:    ['Mini Batch']['Size']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Mini Batch", "Strategy"))
  {
    try
    {
      _miniBatchStrategy = js["Mini Batch"]["Strategy"].get<std::string>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Time Sequence Length"))
  {
    try
    {
      _timeSequenceLength = js["Time Sequence Length"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "L2 Regularization", "Importance"))
  {
    try
    {
      _l2RegularizationImportance = js["L2 Regularization"]["Importance"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Discount Factor"))
  {
    try
    {
      _discountFactor = js["Discount Factor"].get<float>();
    } catch (const std::exception& e) {
      KORALI_LOG_ERROR(" + Object: [ agent ] \n + Key:    ['Discount Factor']\n%s", e.what());
    }
  ...
  if (isDefined(js, "Importance Weight Truncation Level"))
  {
    try
    {
      _importanceWeightTruncationLevel = js["Importance Weight Truncation Level"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Serialize"))
  {
    try
    {
      _experienceReplaySerialize = js["Experience Replay"]["Serialize"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Start Size"))
  {
    try
    {
      _experienceReplayStartSize = js["Experience Replay"]["Start Size"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Maximum Size"))
  {
    try
    {
      _experienceReplayMaximumSize = js["Experience Replay"]["Maximum Size"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "Cutoff Scale"))
  {
    try
    {
      _experienceReplayOffPolicyCutoffScale = js["Experience Replay"]["Off Policy"]["Cutoff Scale"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "Target"))
  {
    try
    {
      _experienceReplayOffPolicyTarget = js["Experience Replay"]["Off Policy"]["Target"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "Annealing Rate"))
  {
    try
    {
      _experienceReplayOffPolicyAnnealingRate = js["Experience Replay"]["Off Policy"]["Annealing Rate"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experience Replay", "Off Policy", "REFER Beta"))
  {
    try
    {
      _experienceReplayOffPolicyREFERBeta = js["Experience Replay"]["Off Policy"]["REFER Beta"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Experiences Between Policy Updates"))
  {
    try
    {
      _experiencesBetweenPolicyUpdates = js["Experiences Between Policy Updates"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "State Rescaling", "Enabled"))
  {
    try
    {
      _stateRescalingEnabled = js["State Rescaling"]["Enabled"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Reward", "Rescaling", "Enabled"))
  {
    try
    {
      _rewardRescalingEnabled = js["Reward"]["Rescaling"]["Enabled"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Multi Agent Relationship"))
  {
    try
    {
      _multiAgentRelationship = js["Multi Agent Relationship"].get<std::string>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Multi Agent Correlation"))
  {
    try
    {
      _multiAgentCorrelation = js["Multi Agent Correlation"].get<int>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Multi Agent Sampling"))
  {
    try
    {
      _multiAgentSampling = js["Multi Agent Sampling"].get<std::string>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Termination Criteria", "Max Episodes"))
  {
    try
    {
      _maxEpisodes = js["Termination Criteria"]["Max Episodes"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Termination Criteria", "Max Experiences"))
  {
    try
    {
      _maxExperiences = js["Termination Criteria"]["Max Experiences"].get<size_t>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Termination Criteria", "Max Policy Updates"))
  {
    try
    {
      _maxPolicyUpdates = js["Termination Criteria"]["Max Policy Updates"].get<size_t>();
    } catch (const std::exception& e) {
