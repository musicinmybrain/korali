...
void Agent::attendWorker(size_t workerId)
{
  ...
  if (_workers[workerId].retrievePendingMessage(message))
  {
    ...
    if (message["Action"] == "Train Policy")
    {
      ...
      const auto stateValues = message["State Value"].get<std::vector<float>>();
      ...
      _gradientQueue.push(trainPolicy(miniBatch, stateValues, distributionParams));
...
void Agent::processEpisode(knlohmann::json &episode)
{
  ...
  for (size_t expId = 0; expId < episodeExperienceCount; expId++)
  {
    ...
    {
      KORALI_LOG_ERROR("Distribution Parameters missing for the current experience.\n");
    }
  ...
  {
    KORALI_LOG_ERROR("Truncated State deprecated for external policy mode");
  }
