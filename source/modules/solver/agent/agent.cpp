...
void Agent::processEpisode(knlohmann::json &episode)
{
  ...
  for (size_t expId = 0; expId < episodeExperienceCount; expId++)
  {
    ...
    if (_rewardRescalingEnabled)
    {
      ...
      {
        for (size_t d = 0; d < _problem->_agentsPerEnvironment; d++)
          _rewardRescalingSumSquaredRewards -= _rewardVectorContiguous[0+d] * _rewardVectorContiguous[0+d];
      }
      for (size_t d = 0; d < _problem->_agentsPerEnvironment; d++)
      {
  ...
  for (ssize_t expId = endId; expId >= startId; expId--)
  {
    for (size_t d = 0; d < _problem->_agentsPerEnvironment; d++)
    {
      ...
      retV[d] = getScaledReward(_rewardVectorContiguous[expId+d]) + _discountFactor * retV[d];
      _retraceValueVectorContiguous[expId+d] = retV[d];
    }
  }
...
#pragma omp parallel for
    for (size_t i = 0; i < updateBatch.size(); i++)
    {
      ...
      for (size_t d = 0; d < numAgents; d++)
        averageStateValue += _stateValueVectorContiguous[expId + d];
  ...
  for (size_t d = 0; d < numAgents; d++)
  {
    ...
    if (_problem->_policiesPerEnvironment == 1)
      _experienceReplayOffPolicyRatio[d] /= (float)numAgents;
  }
