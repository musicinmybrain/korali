...
float Continuous::calculateImportanceWeight(const std::vector<float> &action, const policy_t &curPolicy, const policy_t &oldPolicy)
{
  ...
  if( _minimalApproximation )
  {
    ...
    logpOldPolicy = oldPolicy.actionProbabilities[0];
    logpCurPolicy = curPolicy.actionProbabilities[0];
  }
  else {
    if (_policyDistribution == "Normal")
    {
...
float Continuous::calculateLogActionProbability(const std::vector<float> &action, const policy_t& policy) const
{
  ...
  if (_policyDistribution == "Normal")
  {
    for (size_t i = 0; i < action.size(); i++)
    {
      // Getting parameters from policy
      const float curMean = policy.distributionParameters[i];
      const float curSigma = policy.distributionParameters[_problem->_actionVectorSize + i];
  ...
  if (_policyDistribution == "Squashed Normal")
  {
    KORALI_LOG_ERROR("Squashed Normal is not supported with _minimalApproximation.\n");
  }
  ...
  if (_policyDistribution == "Clipped Normal")
  {
    ...
    {
      // Getting parameters from policy
      const float curMu = policy.distributionParameters[i];
      const float curSigma = policy.distributionParameters[_problem->_actionVectorSize + i];
  ...
  if (_policyDistribution == "Beta")
  {
    for (size_t i = 0; i < action.size(); i++)
    {
      // Getting parameters from policy
      const float curMu = policy.distributionParameters[i];
      const float curVariance = policy.distributionParameters[_problem->_actionVectorSize + i];
...
std::vector<float> Continuous::calculateImportanceWeightGradient(const std::vector<float> &action, const policy_t &curPolicy, const policy_t &oldPolicy, const float importanceWeight)
{
  ...
  {
    // Read old/current probabilities
    const float oldLogProbability = oldPolicy.actionProbabilities[0];
    const float curLogProbability = calculateLogActionProbability(action, curPolicy);

    // Compute log-factor
    const float logFactor = curLogProbability - oldLogProbability;

    // Compute factor
    factor = std::exp(logFactor);
  }
