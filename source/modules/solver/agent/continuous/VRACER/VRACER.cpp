...
void VRACER::initializeAgent()
{
  ...
  for (size_t p = 0; p < _problem->_policiesPerEnvironment; p++)
  {
    ...
    _criticPolicyExperiment[p]["Problem"]["Solution"]["Size"] = 1;
...
#pragma omp parallel for schedule(guided, numAgents) reduction(vec_float_plus \
  ...
  for (size_t b = 0; b < miniBatchSize; b++)
  {
    ...
    if (_multiAgentRelationship == "Cooperation")
      gradientStateValue[0] /= numAgents;
    ...
    if (_isOnPolicyBuffer[expId][agentId])
    {
      ...
      for (size_t i = 0; i < _policyParameterCount; i++)
        gradientPolicyParams[b][i] = _currentLearningRate * _experienceReplayOffPolicyREFERCurrentBeta[agentId] * lossOffPolicy * polGrad[i];
    }
    ...
    _criticPolicyProblem[policyIdx]->_solutionData[b] = gradientStateValue;
