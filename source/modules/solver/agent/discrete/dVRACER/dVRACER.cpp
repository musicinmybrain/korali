...
#pragma omp parallel for schedule(guided, numAgents) reduction(+ \
  ...
  for (size_t b = 0; b < miniBatchSize; b++)
  {
    ...
    if (_isOnPolicyVector[expId][agentId])
    {
      ...
      {
        float nextExpVtbc = _retraceValueVectorContiguous[(expId + 1) * numAgents + agentId];
...
void dVRACER::setConfiguration(knlohmann::json& js) 
{
  ...
  if (isDefined(js, "Statistics", "Average Inverse Temperature"))
  {
    try
    {
      _statisticsAverageInverseTemperature = js["Statistics"]["Average Inverse Temperature"].get<float>();
  ...
  if (isDefined(js, "Statistics", "Average Action Unlikeability"))
  {
    try
    {
      _statisticsAverageActionUnlikeability = js["Statistics"]["Average Action Unlikeability"].get<float>();
    } catch (const std::exception& e) {
  ...
  if (isDefined(js, "Initial Inverse Temperature"))
  {
    try
    {
      _initialInverseTemperature = js["Initial Inverse Temperature"].get<float>();
    } catch (const std::exception& e) {
