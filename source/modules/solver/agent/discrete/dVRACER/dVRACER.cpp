...
void dVRACER::initializeAgent()
{
  ...
  for (size_t p = 0; p < _problem->_policiesPerEnvironment; p++)
  {
    ...
    _criticPolicyExperiment[p]["Problem"]["Solution"]["Size"] = 1 + _policyParameterCount[p]; // The value function, action q values, and inverse temperatur
    ...
    for (size_t i = 0; i < _problem->_actionCount[p]; ++i)
    {
    ...
    _criticPolicyExperiment[p]["Solver"]["Neural Network"]["Output Layer"]["Transformation Mask"][1 + _problem->_actionCount[p]] = "Softplus"; // x = 0.5 * (x + std::sqrt(1. + x * x));
    _criticPolicyExperiment[p]["Solver"]["Neural Network"]["Output Layer"]["Scale"][1 + _problem->_actionCount[p]] = 1.0f;
    _criticPolicyExperiment[p]["Solver"]["Neural Network"]["Output Layer"]["Shift"][1 + _problem->_actionCount[p]] = _initialInverseTemperature - 0.5;
...
#pragma omp parallel for schedule(guided, numAgents) reduction(+ \
  ...
  for (size_t b = 0; b < miniBatchSize; b++)
  {
    ...
    if (_isOnPolicyBuffer[expId][agentId])
    {
      ...
      auto polGrad = calculateImportanceWeightGradient(curPolicy, expPolicy, policyIdx);
    ...
    auto klGrad = calculateKLDivergenceGradient(expPolicy, curPolicy, policyIdx);
    ...
    // Update statistics
    _statisticsAverageInverseTemperature += (curPolicy.distributionParameters[_problem->_actionCount[policyIdx]] / (float)_problem->_policiesPerEnvironment);

    float unlikeability = 1.0;
    for (size_t i = 0; i < _problem->_actionCount[policyIdx]; ++i)
...
#pragma omp parallel for
  for (size_t b = 0; b < batchSize; b++)
  {
    ...
    const float invTemperature = evaluation[b][_policyParameterCount[policyIdx]];

    // Storage for Q(s,a_i) and max_{a_i} Q(s,a_i)
    std::vector<float> qValAndInvTemp(_policyParameterCount[policyIdx]);
    ...
    std::vector<float> pActions(_problem->_actionCount[policyIdx]);
    ...
    qValAndInvTemp[_problem->_actionCount[policyIdx]] = invTemperature;
