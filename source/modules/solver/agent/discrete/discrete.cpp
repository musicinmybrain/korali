...
float Discrete::evaluateTrajectoryLogProbability(const std::vector<std::vector<std::vector<float>>> &states, const std::vector<std::vector<std::vector<float>>> &actions, const std::vector<float> &policyHyperparameter)
{
  ...
  for (size_t t = 0; t < states.size(); ++t)
  {
    ...
    const auto &pActions = policyInfo[0].distributionParameters;
...
float Discrete::evaluateTrajectoryLogProbabilityWithObservedPolicy(const std::vector<std::vector<std::vector<float>>> &states, const std::vector<std::vector<std::vector<float>>> &actions)
{
  float trajectoryLogProbability = 0.0;
