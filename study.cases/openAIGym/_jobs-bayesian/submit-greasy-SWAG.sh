...
# create run-folder
FOLDERNAME=${SCRATCH}/korali/swag
mkdir -p ${FOLDERNAME}
    ...
    do
        ...
        do
            ...
            do
                ...
                cat << EOF >> tasks.txt
[@ ${RUNFOLDER}/ @] python run-b-vracer.py --env $E --run $R --nPolicies $NP --nSGD $NSGD --bSWAG 1 > run${R}.txt
EOF
                let NUMNODES++
            done;
        done;
...
#!/bin/bash -l
#SBATCH --account=s929
#SBATCH --job-name=swag
#SBATCH --output=swag_out_%j.txt
#SBATCH --error=swag_err_%j.txt
